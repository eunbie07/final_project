...
export const FurnitureModel = React.memo(function FurnitureModel({
            ...
            try {
              if (clonedMesh.material) {
                ...
                if (Array.isArray(clonedMesh.material)) {
                  ...
                  clonedMesh.material = setupMaterial(newMaterial, selected, hasCollision);
                }
              } else {
                // 재질이 없는 경우 기본 재질 생성
                clonedMesh.material = new THREE.MeshStandardMaterial({
                  color: selected ? '#ffffff' : hasCollision ? '#ff9999' : '#cccccc',
                  roughness: 0.7,
                  metalness: 0.1
                });
              }
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 기본 바닥 위치 + 오프셋 적용
      const offsetY = modelConfig.offset ? modelConfig.offset[1] : 0;
      group.current.position.y = -scaledBox.min.y + offsetY;
      
    } catch (err) {
      console.error('Error scaling 3D model:', err);
    }
  }, [modelMeshes, size, modelConfig.scale, modelConfig.rotation]);
